 ...
 *
 * What the list shows is the `control.list-tenants` Operation's projection:
 * the platform view of a tenant — its lifecycle, whether provisioning linked
 * a Keycloak Organization (the alias), and its Service catalog counts — not
 * the registry row's ids and timestamps.
 *
              ...
              {result.tenants.map((tenant) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-[var(--color-foreground-muted)]">
                    {tenant.installedEntries} installed
                    {tenant.overriddenEntries > 0 ? `, ${tenant.overriddenEntries} overridden` : ""}
                    {tenant.updatesAvailable > 0 ? `, ${tenant.updatesAvailable} pending` : ""}
                  </td>
                </tr>
              ))}
            </tbody>
