...
describe("Retry-After", () => {
  it("retains only the two legal syntactic forms", () => {
    ...
    // Eleven digits is not a delay, it is a way to smuggle an arbitrary value.
    expect(retainableRetryAfter("12345678901")).toBeUndefined();
  });
...
describe("the failure envelope", () => {
  ...
  it("declares a closed schema whose fields are the envelope's and nothing more", () => {
    expect(Object.keys(FAILURE_ENVELOPE_SCHEMA.properties).sort()).toEqual(
      ["category", "code", "correlationId", "message", "requiredAction", "retryAt", "retryable"],
    );
    expect(FAILURE_ENVELOPE_SCHEMA.additionalProperties).toBe(false);
  });

  // REST and MCP both go through toHttpError, so proving it once proves the
  // two transports mean the same thing.
  it("reaches the REST mapper from both error classes with the same body", () => {
    ...
    const fromServices = new ProviderOutcomeError(outcome, "Rate limited.", 429);
    for (const error of [fromExecutor, fromServices]) {
    ...
    expect(providerOutcomeOf(fromServices)).toEqual(outcome);
