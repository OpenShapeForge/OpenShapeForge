...
export function retainableRetryAfter(value: string | null | undefined): string | undefined {
  ...
  if (trimmed === "" || trimmed.length > MAX_RETRY_AFTER_LENGTH) return undefined;
  if (/^[+-]?\d+(\.\d+)?$/.test(trimmed)) {
    return /^\d{1,10}$/.test(trimmed) ? trimmed : undefined;
  }
...
export function retryAtFromRetryAfter(
  ...
  const at = /^\d+$/.test(retained) ? now + Number(retained) * 1000 : Date.parse(retained);
...
export function classifyProviderOutcome(
  ...
  if (hint && hint.status === observation.status) {
    if (hint.code !== undefined && allowed.includes(hint.code)) code = hint.code;
...
/**
 * A provider failure outside the connector package runtime — the authored
 * Service path, which talks to providers from stored rows rather than through
 * a contract. Same outcome, same codes, same redaction: the message is built
 * from the code, never from the response.
 */
...
/** JSON Schema of `FailureBody["error"]`, for MCP `outputSchema` declarations. */
export const FAILURE_ENVELOPE_SCHEMA = {
  type: "object",
  properties: {
    code: { type: "string" },
    message: { type: "string" },
