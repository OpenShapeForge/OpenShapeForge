 ...
 */
/** The cross-tenant definer role, as the generated manifest's role contract names it. */
const BLUEPRINT_READER_ROLE = databaseRole("blueprintReader").name;

export async function applyBlueprintsMigration(db: OpenShapeForgeDatabase) {
  const tenants = sql.table(entityTableName("Tenant"));
  const tenantKind = sql.ref(entityColumnName("Tenant", "tenantKind"));
  ...
  await sql`
    ...
    grant usage on schema app, platform to ${sql.id(BLUEPRINT_READER_ROLE)};
    grant execute on function app.current_tenant(), app.current_user_id() to ${sql.id(BLUEPRINT_READER_ROLE)};
    ...
    create policy blueprint_libraries_reader on platform.blueprint_libraries for select
      to ${sql.id(BLUEPRINT_READER_ROLE)} using (true);
    ...
    create policy blueprint_versions_publish on platform.blueprint_versions for insert
      with check (
        ...
        and ${sql.lit(PLATFORM_OPERATOR_ROLE)} = any(string_to_array(current_setting('app.roles', true), ','))
        -- 'blueprint' is the Tenant.tenantKind value of a blueprint tenant.
        and exists (select 1 from ${tenants} t where t.tenant_id = app.current_tenant() and t.${tenantKind} = 'blueprint')
      );
    ...
    grant select on platform.blueprint_libraries, platform.blueprint_versions
      to ${sql.id(BLUEPRINT_READER_ROLE)};
    ...
    grant create on schema app to ${sql.id(BLUEPRINT_READER_ROLE)};
    alter function app.read_blueprints(text, text, text, integer, integer)
      owner to ${sql.id(BLUEPRINT_READER_ROLE)};
    revoke create on schema app from ${sql.id(BLUEPRINT_READER_ROLE)};
