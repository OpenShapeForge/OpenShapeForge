 ...
 * itself for a base type and the osf-type registry entry's `valueType`
...
export function storedFieldBaseType(definition: { osfType?: unknown }): string {
  ...
  const semantic = (fieldSchemaRegistry.osfTypes as Record<string, { valueType?: unknown } | undefined>)[osfType];
