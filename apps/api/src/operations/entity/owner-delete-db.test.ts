 ...
 * compiled catalog whose generic delete is enabled, Document → DocumentRevision
 * (document_revisions.document_id). The owned child's own generic delete is
 * refused statically; collection-policy.test.ts covers that.
 */
...
async function seed(children: number) {
  ...
  for (let index = 0; index < children; index++) {
    await sql`insert into erp.document_revisions(id, tenant_id, document_id) values (${randomUUID()}::uuid, ${tenant}::uuid, ${id}::uuid)`.execute(privileged!.db);
  }
  return id;
}
async function count(table: "documents" | "document_revisions") {
...
(adminUrl ? describe : describe.skip)("generic delete of a collection owner against PostgreSQL", () => {
  beforeAll(async () => {
    ...
    await sql`create schema erp; create schema platform;
      ...
      create table erp.document_revisions(id uuid primary key default gen_random_uuid(), tenant_id uuid not null, document_id uuid not null,
        channel text not null default 'pdf', locale text not null default 'nl', status text not null default 'draft',
        created_at timestamptz not null default clock_timestamp(), updated_at timestamptz not null default clock_timestamp(),
        unique(tenant_id,id), foreign key(tenant_id,document_id) references erp.documents(tenant_id,id) on delete cascade);
      ...
      alter table erp.document_revisions enable row level security; alter table erp.document_revisions force row level security;
      create policy tenant on erp.documents using(tenant_id=app.current_tenant()) with check(tenant_id=app.current_tenant());
      create policy tenant on erp.document_revisions using(tenant_id=app.current_tenant()) with check(tenant_id=app.current_tenant());
  ...
  beforeEach(async () => {
    await sql`truncate erp.document_revisions, erp.documents, platform.entity_events`.execute(privileged!.db);
  });
  afterAll(async () => {
  ...
  test("is refused while owned children exist, even though the FK would cascade", async () => {
    ...
    expect(await count("document_revisions")).toBe(2);
  ...
  test("succeeds once the owner has no owned children", async () => {
    ...
    await sql`delete from erp.document_revisions where document_id = ${owner}::uuid`.execute(privileged!.db);
