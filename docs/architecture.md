...
Compiler  (packages/compiler, `bun run generate`)
  ...
  entity YAML contracts  ─┘  compiled once, promoted to tables (erp.*)
        │
        ▼
Generated artifacts (compiler-owned and reproducible; most runtime roots gitignored)
...
generated root or carries a `generated` name and starts with a
"Generated by OpenShapeForge Service Compiler. Do not edit" header where the
format allows one. Bulky runtime roots and host-local output are gitignored;
a small allowlist of shared compiler contract artifacts is committed and
checked for exact freshness. You never edit either category by hand; edit the
...
under the roots in `compilerOwnedGeneratedRoots` is recreated and checked for
staleness or orphans by `bun run generate` and `check:generated` — edit
`packages/compiler`, never the output.
