...
- `isError: true`;
- every connector tool declares an `outputSchema` with `result` (the operation's
  output) and `error` (the failure envelope), and returns
  `structuredContent: { result }` on success — a client that sees an output
  schema expects structured content either way.
