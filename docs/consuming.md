...
- **Scheduled retention is advisory metadata; there is no general enforcement
  runtime.** The compiler emits a `retention` block (clock, rules, legal hold,
  review gates, crypto-delete key, erasure cascades) into the DB manifest, but
  no scheduler deletes/anonymizes data past its window or honors legal holds.
  The API has one fixed Relation erasure procedure that validates its plan
  against this metadata; it is not a generic compiler runtime and hosts do not
  gain a new erasure procedure by adding metadata. See
