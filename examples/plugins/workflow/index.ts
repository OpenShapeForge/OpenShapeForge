...
/**
 * The route file, which is a re-export and nothing else.
 *
 * The page itself is hand-written in `web/`, where `typecheck:web` reads it as
 * source rather than as a string literal. What is generated is only the fact
 * that the route exists — which is precisely the part that has to disappear
 * when the plugin is not registered.
 *
 * The relative path climbs out of apps/web and back into examples/, the same
 * reach the runtime half already makes into apps/api. Emitted, so it is exact
 * by construction rather than by somebody counting `../` correctly.
 */
const WEB_ROUTE_FILES: ReadonlyArray<{ path: string; source: string }> = [
  {
    path: `${WEB_ROUTE_ROOT}/page.tsx`,
    source: "../../../../../../examples/plugins/workflow/web/workflow-page",
  },
];
...
function webRouteArtifacts(): GeneratedArtifact[] {
  return WEB_ROUTE_FILES.map((route) => ({
    ...
    contents:
      ...
      `export { default } from "${route.source}";\n`,
  }));
}
