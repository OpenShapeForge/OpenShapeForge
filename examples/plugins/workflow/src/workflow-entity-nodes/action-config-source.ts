...
export function buildConfigFieldsWithErrorRoutesSource(input: {
  action: WorkflowAction;
  constPrefix: string;
  readableFields: Field[];
  defaultSort?: { field: string; direction: "asc" | "desc" };
}): string {
  const { action, constPrefix, readableFields, defaultSort } = input;
  const awaitActionConfigFieldsSource = `[
  ...
  const configFieldsWithErrorRoutesSource =
    action === "wait"
      ? `[
    ${constPrefix}RecordIdField,
    ...
    ${constPrefix}WaitTimeoutField,
      ...
      : action === "awaitAction"
      ? awaitActionConfigFieldsSource
      : action === "getOne" || action === "delete"
      ? `[${constPrefix}RecordIdField]`
      : action === "update"
        ? `[
    ${constPrefix}RecordIdField,
    {
      ...
      children: ${constPrefix}WritableFieldsOptional,
    },
  ]`
        : action === "create"
          ? `[
    {
      ...
      children: ${constPrefix}WritableFields,
    },
  ]`
          : (() => {
              const sortableFieldOptions = readableFields
                ...
                .map((field) => ({
                  value: field.key,
                  label:
                    field.label ?? { nl: field.key, en: field.key },
                }));
              const sortFieldDefault = defaultSort
                ? `defaultValue: ${JSON.stringify(defaultSort.field)},`
                : "";
              const sortDirectionDefault = defaultSort
                ? `defaultValue: ${JSON.stringify(defaultSort.direction)},`
                : `defaultValue: "asc",`;
              return `[
    {
      key: "limit",
      ...
      children: [
        {
          ...
          options: {
            type: "static",
            items: ${JSON.stringify(sortableFieldOptions)},
          },
          render: {
  ...
  return configFieldsWithErrorRoutesSource;
}
