...
describe("applyAuthorizationPatch — renameClient", () => {
  test("rewrites every reference to the client id and nothing else", () => {
    const merged = apply({ renameClient: { from: "erp-provider", to: "acme-api" } });

    expect(merged.keycloak.entityRoleClient).toBe("acme-api");
    ...
    expect(merged.realmRoles.controller!.composites).toEqual({
      "acme-api": ["Finance.All.ReadWrite"],
    });
    expect(Object.keys(merged.clientRoles)).toEqual(["acme-api", "openshapeforge-support"]);
    expect(merged.users[0]!.clientRoles).toEqual({ "acme-api": ["Relations.All.ReadWrite"] });
  ...
  test("the patch body addresses the client under its NEW id", () => {
    ...
    const client = merged.keycloak.clients.find((c) => c.id === "acme-api")!;
  ...
  test("renames serviceAccountClientRoles keys and the v1 keycloak.client / keycloak.realmRoles shape", () => {
    ...
    expect(renamed.keycloak.client).toBe("acme-api");
    expect(renamed.keycloak.realmRoles.legacy!.composites).toEqual({ "acme-api": ["X"] });
    expect(renamed.keycloak.clients[1]!.serviceAccountClientRoles).toEqual({
      "acme-api": ["General.All.Read"],
    });
  });

  test("a rename of a client the base does not have is refused", () => {
    expect(() => apply({ renameClient: { from: "nope", to: "acme-api" } })).toThrow(
  ...
  test("renameClient only knows from and to", () => {
    expect(() =>
      apply({ renameClient: { from: "erp-provider", to: "acme-api", name: "Acme API" } }),
...
describe("applyAuthorizationPatch — merge", () => {
  ...
  test("inputs are not mutated", () => {
    ...
    apply({ renameClient: { from: "erp-provider", to: "acme-api" }, clientRoles: { "acme-api": ["X"] } }, base);
...
describe("resolveAuthoringLayers — authorizationPatch", () => {
  ...
  test("a host layer renames the entity-role client through to the generated realm export", () => {
    ...
    expect(realm.clients.map((c: { clientId: string }) => c.clientId)).toContain("acme-api");
    const client = realm.clients.find((c: { clientId: string }) => c.clientId === "acme-api");
    expect(client.name).toBe("Acme API");
    expect(client.secret).toBe("acme-api-secret");
    ...
    expect(gateway.protocolMappers.map((m: { name: string }) => m.name)).toContain("acme-api-audience");
    expect(Object.keys(realm.roles.client)).toContain("acme-api");
    const directie = realm.roles.realm.find((r: { name: string }) => r.name === "directie");
    expect(Object.keys(directie.composites.client)).toEqual(["acme-api", "openshapeforge-support"]);
  });
  ...
  test("patches stack in layer order, each seeing the previous result", () => {
    ...
    expect(merged.clientRoles["acme-api"]).toHaveLength(4);
  });
  ...
  test("a second realm file is patched under its own name and the first is left alone", () => {
    ...
    writeYaml(root, "host/authorization.control.yaml", {
      kind: "authorizationPatch",
      realm: { displayName: "Acme Control" },
    });
    configureLayers(root, ["base", "host"]);

    const resolved = resolveAuthoringLayers(root);
    const control = YAML.parse(readFileSync(join(resolved, "authorization.control.yaml"), "utf8"));
    expect(control.realm).toEqual({ name: "openshapeforge-control", displayName: "Acme Control" });
  ...
  test("a malformed patch fails at resolve time with the patch's own path", () => {
    ...
    writeYaml(root, "host/authorization.yaml", {
      kind: "authorizationPatch",
      renameClient: { from: "missing-client", to: "acme-api" },
    });
    configureLayers(root, ["base", "host"]);
