...
import type { Field, EntityProfile } from "../types.js";
...
describe("resolveStorageColumns identifier validation", () => {
  ...
  it("validates the foreign-key column of a single entity reference", () => {
    expect(() =>
      resolveStorageColumns(
        [{ ...field("owner", 'owner_id" --'), osfType: "User", relationship: { kind: "belongsTo", target: "User", foreignKey: 'owner_id" --' } }],
        [],
      ),
    ).toThrow(/Unsafe storage column name/);
  });
