...
test("entity type discovery is a trusted canonical native read operation with authored labels", () => {
  ...
  expect(operations[0]).toMatchObject({ key: "entityTypes.list", auth: { mode: "session" }, effects: { data: "read", external: "none" }, implementation: { type: "entity-type-list", labels: { Example: { nl: "Voorbeeld" } } } });
