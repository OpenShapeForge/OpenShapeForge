...
describe("rich generated REST OpenAPI", () => {
  it("introduces OSF with a safe developer start flow and reusable prompt", () => {
    const description = spec().info.description;

    expect(description).toContain(
      "OpenShapeForge is an extensible, compiler-driven core ERP platform.",
    );
    expect(description).toContain("### Start here");
    expect(description).toContain("Begin with its list operation");
    expect(description).toContain(
      "### Starter prompt for a developer or coding assistant",
    );
    expect(description).toContain(
      "Treat this OpenAPI document as the source of truth.",
    );
    expect(description).toContain("confirm the write with the user");
