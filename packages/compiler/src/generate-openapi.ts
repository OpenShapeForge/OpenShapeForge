...
const GENERIC_DEVELOPER_ONBOARDING = [
  ...
  "## Start here",
  "",
  "1. **Authorize.** Every operation needs a bearer token — use the **Authorize**",
  "   button above. Authorization is separate from the schema: the caller's roles",
  "   decide which documented entities and fields may be read or written, so a",
  "   contract-valid request can still receive `401` or `403`.",
  "2. **Pick an entity.** Each tag below describes one authored entity and its",
  "   available operations.",
  `3. **Read a few records first.** \`GET ${REST_MOUNT}/{entity}?first=5\` is the`,
  "   safest first call. Inspect the page and its cursor before following it.",
  ...
  "   `{Entity}UpdateInput` is the partial-update body. Use their required fields,",
  "   validation, and enum values exactly as documented.",
  "",
  "## Starter prompt",
  "",
  ...
  "  contract may still be refused with 401 or 403. Report that answer; do not try",
  "  to work around it.",
