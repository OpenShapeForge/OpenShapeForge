...
function openApiDescription(source: string): string {
  return `OpenShapeForge is an extensible, compiler-driven core ERP platform. Entity definitions are authored once and compiled into tenant-aware REST contracts, validation, data models, and runtime behavior.

This generated OpenAPI document is the source of truth for the REST surface available in this deployment: paths, methods, schemas, required and read-only fields, enum values, filters, cursor pagination, and sorting.

### Start here

1. Select **Authorize** and provide a valid bearer token. The documentation describes the contract; it does not grant access.
2. Choose an entity tag and inspect its response and input schemas.
3. Begin with its list operation using a small **first** value, then fetch one record by **id**.
4. Before writing, inspect the create or update input schema and its required, read-only, validation, and enum constraints.

### Starter prompt for a developer or coding assistant

~~~text
You are integrating with an OpenShapeForge-generated REST API. Treat this OpenAPI document as the source of truth. First identify the relevant entity tag and schema, then start with a small read-only list request. Use only documented paths, methods, fields, enum values, filters, pagination, and sort options. Before any create, update, or delete request, show the proposed operation and JSON body, verify required and read-only fields, and confirm the write with the user. Never invent contract details.
~~~
...
export function renderOpenApiSpec(
  ...
  const spec = {
    ...
    info: {
      ...
      description: openApiDescription(source),
      version: "1",
    },
    security: [{ bearerAuth: [] }],
    components: {
