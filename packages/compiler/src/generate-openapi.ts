...
/**
 * The header a developer meets first, in `info.description`. OpenAPI has no
 * field for "how do I approach this API", and none for an assistant prompt, so
 * both live here: `Info Object.description` is CommonMark, and Swagger UI
 * renders it above the tag list.
 *
 * Static prose, deliberately. It describes what every generated OSF API has in
 * common — the contract is generated, authorization is separate from it — and
 * says nothing about which entities this particular deployment authored, which
 * is what the tags and schemas below are for. That keeps it assertable by a
 * unit test and keeps deployment-specific values out of a document served
 * unauthenticated.
 */
const DEVELOPER_INTRODUCTION = [
  "OpenShapeForge is an extensible, compiler-driven core ERP platform. Entities are",
  "authored as YAML, and the compiler turns each authored entity into a tenant-aware",
  "REST contract together with the runtime behind it: storage, authorization,",
  "filtering, and this document.",
  "",
  "**This document is generated, and it is the contract.** The paths, schemas,",
  "required fields, enum values, pagination, filtering, and sorting below are the",
  "ones the running server implements. Treat anything absent here as absent from",
  "the API: there are no undocumented endpoints, no extra fields, and no enum",
  "values beyond the ones each schema lists.",
  "",
  "## Start here",
  "",
  "1. **Authorize.** Every operation needs a bearer token — the **Authorize**",
  "   button above. Authorization is *not* part of the schema: the token's roles",
  "   decide which entities and fields you may read or write, so a request that",
  "   matches this contract exactly can still be answered `401` or `403`.",
  "2. **Pick an entity.** Each tag below is one authored entity, described with its",
  "   authored description.",
  `3. **Read a few records first.** \`GET ${REST_MOUNT}/{entity}?first=5\` is the`,
  "   safest first call. A list response is a page: `items`, `totalCount`, and",
  "   `nextCursor`.",
  `4. **Inspect one record.** \`GET ${REST_MOUNT}/{entity}/{id}\`, using an \`id\``,
  "   from that page, shows a single record in full.",
  "5. **Read the input schema before writing.** `{Entity}Input` is the create body:",
  "   it carries the required fields, lengths, patterns, and enums the server",
  "   enforces. `{Entity}UpdateInput` is the PATCH body: partial, and fields",
  "   authored immutable are create-only and rejected there.",
  "",
  "## Starter prompt",
  "",
  ...
  "  endpoints, fields, or query parameters, and do not carry over conventions from",
  "  other APIs you have seen.",
  ...
  "  ({Entity}Input to create, {Entity}UpdateInput to partially update). Send",
  "  nothing a schema does not allow.",
  "- Authorization is not described by the schema. A request that matches the",
  "  contract may still be refused with 401 or 403 depending on the caller's roles.",
  "  Report that as the API's answer; do not try to work around it.",
  "- Stay on read-only requests (GET) while exploring, and confirm with me before",
  "  any POST, PATCH, or DELETE.",
  ...
  const spec = {
    ...
    info: {
      ...
      description: [
        ...
        "",
        `Generated by @openshapeforge/compiler. Source: ${source}. Do not edit by hand.`,
      ].join("\n"),
      version: "1",
    },
    security: [{ bearerAuth: [] }],
