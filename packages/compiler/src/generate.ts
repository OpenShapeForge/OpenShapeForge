...
/**
 * The key a tenant-scoped table is identified by WITHIN its tenant.
 *
 * Returns the authored single-column primary key (`id`, and for a handful of
 * hash-keyed tables something else) of a tenant-scoped table, or undefined for
 * a global table or one whose primary key is a versioned table constraint.
 *
 * This is the pivot of the tenant-consistent-key design (issue #509). A
 * PostgreSQL foreign-key check runs as the table OWNER and is explicitly
 * exempt from row level security, so a single-column `REFERENCES parent(id)`
 * is satisfied by a parent row in ANOTHER tenant — RLS never sees the lookup.
 * Making the key `(tenant_id, id)` and every reference to it
 * `(tenant_id, child_col) -> (tenant_id, id)` reusing the CHILD's own
 * tenant_id column removes the column a foreign tenant id could be written
 * into: a cross-tenant reference stops being forbidden and becomes
 * unspeakable.
 *
 * Two consequences the rest of this file depends on:
 *   - `id` alone is no longer unique, so nothing (including a hand-written
 *     plugin migration) can attach a single-column foreign key to it. That is
 *     the point; `assertNoTenantScopedIdentityUnique` keeps it true.
 *   - a referencing table needs a `tenant_id` column of its own. Every
 *     tenant-scoped table has one by construction, and a global table that
 *     points at tenant-scoped data must carry one (platform.api_keys does).
 */
function tenantScopedIdentityColumn(table: TableDefinition): string | undefined {
  if (!table.tenantScoped) return undefined;
  const primaryKeys = table.columns.filter((column) => column.primaryKey === true);
  if (primaryKeys.length !== 1) return undefined;
  const primaryKey = primaryKeys[0]!;
  // A table whose whole primary key IS tenant_id has nothing to compose with.
  if (primaryKey.name === "tenant_id") return undefined;
  return primaryKey.name;
}

function renderColumnSql(column: ColumnDefinition, inlinePrimaryKey: boolean): string {
  const parts = [quoteIdent(column.name), sqlType(column.type)];

  if (column.generated === "identity") {
    parts.push("GENERATED BY DEFAULT AS IDENTITY");
  }
  if (column.primaryKey && inlinePrimaryKey) {
...
function renderTableSql(table: TableDefinition): string {
  ...
  // Tenant-scoped tables key on (tenant_id, <id>) rather than on <id> alone;
  // see tenantScopedIdentityColumn for why. The key then has to be a
  // table-level constraint, so the column loses its inline PRIMARY KEY (it
  // keeps NOT NULL, which the composite key would impose anyway).
  const identityColumn = tenantScopedIdentityColumn(table);
  const columnLines = table.columns
    .map((column) => renderColumnSql(column, identityColumn === undefined))
    .join(",\n");
  const body =
    identityColumn === undefined
      ? columnLines
      : `${columnLines},\n  PRIMARY KEY (${quoteIdent("tenant_id")}, ${quoteIdent(identityColumn)})`;
  const lines = [
...
function renderColumnForeignKeySql(
  table: TableDefinition,
  ...
  const target = tables.get(`${reference.schema}.${reference.table}`);
  const targetIdentity = target ? tenantScopedIdentityColumn(target) : undefined;
  // A reference into a tenant-scoped table's identity travels through the
  // referencing row's OWN tenant_id, so there is no column left in which a
  // foreign tenant could be named. A reference into a global table (a
  // reference-data catalog, the tenant registry) keeps its single column: a
  // global row belongs to everyone, so there is nothing to keep consistent.
  const tenantConsistent = targetIdentity !== undefined && targetIdentity === reference.column;

  if (tenantConsistent && !columnNames(table).has("tenant_id")) {
    throw new Error(
      `Table ${table.schema}.${table.name} references tenant-scoped ${reference.schema}.${reference.table}` +
        ` through "${column.name}" but has no tenant_id column of its own, so the reference cannot be made` +
        ` tenant-consistent. Add tenant_id to the table, or point the reference at a global table.`,
    );
  }

  const columns = tenantConsistent
    ? `(${quoteIdent("tenant_id")}, ${quoteIdent(column.name)})`
    : `(${quoteIdent(column.name)})`;
  const referencedColumns = tenantConsistent
    ? `(${quoteIdent("tenant_id")}, ${quoteIdent(reference.column)})`
    : `(${quoteIdent(reference.column)})`;
  // SET NULL over a composite key would null the tenant_id half as well, which
  // would move the row out of its own tenant. Postgres 17's column list
  // restricts the action to the referencing column. (PG 15+; we require 17.)
  const onDelete = reference.onDelete
    ? reference.onDelete === "SET NULL" && tenantConsistent
      ? ` ON DELETE SET NULL (${quoteIdent(column.name)})`
  ...
  return [
    ...
    `      ADD CONSTRAINT ${quoteIdent(constraintName)} FOREIGN KEY ${columns}`,
    `      REFERENCES ${quoteIdent(reference.schema)}.${quoteIdent(reference.table)}${referencedColumns}${onDelete};`,
    "  END IF;",
    "END",
    "$openshapeforge_fk$;",
  ].join("\n");
...
/**
 * A unique index on a tenant-scoped table's identity column ALONE reopens the
 * hole (tenant_id, id) closes: it is a key a single-column foreign key can
 * attach to, and such a key is checked with RLS bypassed. Refuse it at build
 * time rather than discovering it as a cross-tenant reference later.
 */
function assertNoTenantScopedIdentityUnique(manifest: PlatformSchemaManifest): void {
  for (const table of manifest.tables) {
    const identityColumn = tenantScopedIdentityColumn(table);
    if (identityColumn === undefined) continue;
    for (const index of table.indexes ?? []) {
      if (index.unique !== true) continue;
      if (index.columns.length === 1 && index.columns[0] === identityColumn) {
        throw new Error(
          `Table ${table.schema}.${table.name} declares unique index ${index.name} on "${identityColumn}" alone.` +
            ` On a tenant-scoped table that is a foreign-key target a single-column reference can attach to,` +
            ` and foreign-key checks bypass row level security. Use [tenant_id, ${identityColumn}] instead.`,
        );
      }
    }
  }
...
function renderForeignKeySql(manifest: PlatformSchemaManifest): string {
  assertNoTenantScopedIdentityUnique(manifest);
  const tables = new Map(
    manifest.tables.map((table) => [`${table.schema}.${table.name}`, table] as const),
  );
  return manifest.tables
    .flatMap((table) =>
      table.columns
        .map((column) => renderColumnForeignKeySql(table, column, tables))
