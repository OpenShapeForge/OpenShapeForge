...
import { fail, object, readRevision, uuid } from "./revision-start.js";
 ...
 * published revisions share Document.versions.
 */
export const publishRevision: ModuleOperationHandler = async (input, context) => {
  const { platform, session } = contextServices(context);
  if (!session.tenantId) fail("UNAUTHENTICATED", "Publishing a revision requires a tenant session.");
  const allowed = new Set(["id", "version", "idempotencyKey"]);
  if (Object.keys(input).some((key) => !allowed.has(key))) fail("VALIDATION", "Unknown revision publish input.");
  ...
  if (create.effects.data !== "write" || create.effects.external !== "none" || getVersion.effects.data !== "read" || getVersion.effects.external !== "none") fail("OPERATION_UNAVAILABLE", "The document version lifecycle has incompatible effects.");
  if (create.input.kind !== "json-schema") fail("OPERATION_UNAVAILABLE", "Document version creation requires its canonical input schema.");
  ...
  const value = await platform.db.withSession(session, async (trx) => {
    ...
    if (!revision) fail("NOT_FOUND", "The revision no longer exists.");
    if (revision!.status === "published" || revision!.status === "superseded") fail("INVALID_STATE", `A ${revision!.status} revision cannot be published again.`);
    ...
    if (artifact.mediaType !== "application/json" || artifact.sha256 !== checksum || artifact.byteSize !== bytes.byteLength) fail("HANDLER_CONTRACT_VIOLATION", "Storage did not verify the exact JSON revision bytes.");
    ...
    await rows(trx, `update erp.document_revisions set status = 'superseded', ${TOUCH} where tenant_id = app.current_tenant() and document_id = $1::uuid and status = 'published' and id <> $2::uuid`, [documentId, id]);
    ...
    await platform.events.append(session, {
      aggregateType: "DocumentRevision", aggregateId: id, eventType: "published",
      payload: { documentId, documentVersionId, artifactId: artifact.artifactId, compositionHash: content.compositionHash },
    });
    return {
