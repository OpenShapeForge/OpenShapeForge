...
import { appendRecordEvent, blockColumns, blockContent, insertRevisionBlock, readTemplateVersion, templateParameterFields, templateVariantBlocks, withRevisionCommand } from "./revision-blocks.js";
...
const REVISION_COLUMNS: Readonly<Record<string, string>> = {
  ...
  published_version_id: "publishedVersionId", follow_error: "followError",
};
...
export const startRevision: ModuleOperationHandler = async (input, context) => {
  ...
  if (!session.tenantId) refuse("UNAUTHENTICATED", "Starting a revision requires a tenant session.");
  const allowed = new Set(["documentId", "templateVersionId", "channel", "locale", "parameters"]);
  if (Object.keys(input).some((key) => !allowed.has(key))) refuse("VALIDATION", "Unknown revision input.");
  ...
  if (typeof channel !== "string" || !(REVISION_CHANNELS as readonly string[]).includes(channel)) refuse("VALIDATION", "Unsupported revision channel.");
  const locale = input.locale;
  if (typeof locale !== "string" || !LOCALE.test(locale)) refuse("VALIDATION", "Invalid revision locale.");
  ...
  const value = await platform.db.withSession(session, async (trx) => {
    ...
    if (templateVersionId) {
      ...
      if (!blocks) refuse("DEPENDENCY_UNRESOLVED", "The template version has no variant for this channel and locale.");
      ...
      const permitted = new Set(platform.schemas.entityValues?.collection("DocumentRevision", "blocks")?.allowedDefinitions ?? []);
      seed = blocks!.filter((node) => permitted.has(String(node.row.definition_key))).map((node) => ({ templateBlockId: rowId(node), row: node.row }));
    }
    ...
    if (!revision) refuse("OPERATION_UNAVAILABLE", "The revision could not be stored.");
    const columns = await blockColumns(trx);
    await withRevisionCommand(trx, "start", async () => {
    ...
    await appendRecordEvent(platform, session, { aggregateType: "documentRevision", table: "document_revisions", id: revision!.id, operation: "created" });
