    ...
    <div class="${properties.kcFormClass!}">
        ...
        <script type="module">
            ...
            registerButton.addEventListener("click", function() {
                ...
                const clientPlatform = navigator.userAgentData?.platform || navigator.platform || "";
                const registrationTime = new Intl.DateTimeFormat(
                    document.documentElement.lang || undefined,
                    { dateStyle: "medium", timeStyle: "short" },
                ).format(new Date());
                const suggestedLabel = [realmDisplayName, clientPlatform, registrationTime]
                    .filter(Boolean)
                    .join(" — ");
                const input = {
